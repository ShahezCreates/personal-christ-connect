...
CREATE TABLE
  attendance_records (
    ...
    sessions_held SMALLINT NOT NULL CHECK (sessions_held >= 0),
    sessions_attended SMALLINT NOT NULL CHECK (
      sessions_attended >= 0
      AND sessions_attended <= sessions_held
    ),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now (),
...
CREATE TABLE
  clubs (
    ...
    level TEXT NOT NULL CHECK (level IN ('university', 'department')),
    school TEXT,
    department TEXT,
    description TEXT,
    official_url TEXT,
